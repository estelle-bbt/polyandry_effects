...
get_stat_flower <- function(data_final_flower, include_contact = FALSE){
  ...
  if(include_contact){
    ...
    model_q_oms_flower_0 <- lme4::lmer(data = data_final_flower, ratio_q ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    model_q_oms_flower_1 <- lme4::lmer(data = data_final_flower, ratio_q ~ oms + ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    model_q_oms_flower_2 <- lme4::lmer(data = data_final_flower, ratio_q ~ oms * ttt + pl + ttt + contact * ttt + (1|session) + (1|session:id))
    model_q_oms_flower_3 <- lme4::lmer(data = data_final_flower, ratio_q ~ oms * ttt + pl * ttt + contact + ttt + (1|session) + (1|session:id))
    model_q_oms_flower_4 <- lme4::lmer(data = data_final_flower, ratio_q ~ oms + ttt + pl + contact + (1|session) + (1|session:id))
    model_q_oms_flower_4b <- lme4::lmer(data = data_final_flower |> filter(!is.na(pl)), ratio_q ~ oms + ttt + pl + contact + (1|session) + (1|session:id))
    model_q_oms_flower_5 <- lme4::lmer(data = data_final_flower, ratio_q ~ ttt + pl + contact + (1|session) + (1|session:id))
    model_q_oms_flower_6 <- lme4::lmer(data = data_final_flower, ratio_q ~ oms + pl + contact + (1|session) + (1|session:id))
    model_q_oms_flower_7 <- lme4::lmer(data = data_final_flower |> filter(!is.na(pl)), ratio_q ~ oms + ttt + contact + (1|session) + (1|session:id))
    model_q_oms_flower_8 <- lme4::lmer(data = data_final_flower, ratio_q ~ oms + ttt + pl + (1|session) + (1|session:id))
    lrt_interoms_q_oms <- anova(model_q_oms_flower_0, model_q_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_q_oms <- anova(model_q_oms_flower_0, model_q_oms_flower_2) |> as.data.frame() # pl * ttt
    lrt_intercontact_q_oms <- anova(model_q_oms_flower_0, model_q_oms_flower_3) |> as.data.frame() # oms * ttt
    lrt_oms_q_oms <- anova(model_q_oms_flower_4, model_q_oms_flower_5) |> as.data.frame() # oms
    lrt_ttt_q_oms <-anova(model_q_oms_flower_4, model_q_oms_flower_6) |> as.data.frame() # ttt
    lrt_pl_q_oms <-anova(model_q_oms_flower_4b, model_q_oms_flower_7) |> as.data.frame() # pl
    lrt_contact_q_oms <-anova(model_q_oms_flower_4, model_q_oms_flower_8) |> as.data.frame() # contact
    
    lrt_q_oms <- lrt_interoms_q_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interoms", .before = 1) |>
      bind_rows(lrt_interpl_q_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interpl", .before = 1)) |>
      ...
      bind_rows(lrt_pl_q_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "pl", .before = 1)) |> 
      bind_rows(lrt_contact_q_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "contact", .before = 1)) |> 
      mutate(method = method_name, proxy = "q", predictor = "oms", .before = 1)
    
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_q_oms_flower_best <- lmerTest::lmer(data = data_final_flower, ratio_q ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    estimate_low_q_oms <- summary(model_q_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","pl","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_q_oms_flower_best <-  lmerTest::lmer(data = data_final_flower, ratio_q ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    estimate_medium_q_oms <- summary(model_q_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("oms","pl","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_q_oms_flower_best <-  lmerTest::lmer(data = data_final_flower, ratio_q ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    estimate_high_q_oms <- summary(model_q_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
    ...
    ### aborted ----
    model_ab_oms_flower_0 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_1 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ oms + ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_2 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ oms * ttt + pl + ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_3 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ oms * ttt + pl * ttt + contact + ttt + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_4 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ oms + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_4b <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_seed,nb_ab) ~ oms + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_4c <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_seed,nb_ab) ~ oms + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_5 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_6 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ oms + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_7 <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_seed,nb_ab) ~ oms + ttt + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_oms_flower_8 <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_seed,nb_ab) ~ oms + ttt + pl + (1|session) + (1|session:id), family = "binomial")
    lrt_interoms_ab_oms <- anova(model_ab_oms_flower_0, model_ab_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_ab_oms <- anova(model_ab_oms_flower_0, model_ab_oms_flower_2) |> as.data.frame() # pl * ttt
    lrt_intercontact_ab_oms <- anova(model_ab_oms_flower_0, model_ab_oms_flower_3) |> as.data.frame() # oms * ttt
    lrt_oms_ab_oms <- anova(model_ab_oms_flower_4, model_ab_oms_flower_5) |> as.data.frame() # oms
    lrt_ttt_ab_oms <-anova(model_ab_oms_flower_4, model_ab_oms_flower_6) |> as.data.frame() # ttt
    lrt_pl_ab_oms <-anova(model_ab_oms_flower_4b, model_ab_oms_flower_7) |> as.data.frame() # pl
    lrt_contact_ab_oms <-anova(model_ab_oms_flower_4c, model_ab_oms_flower_8) |> as.data.frame() # contact
    
    lrt_ab_oms <- lrt_interoms_ab_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interoms", .before = 1) |>
      bind_rows(lrt_interpl_ab_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interpl", .before = 1)) |>
      ...
      bind_rows(lrt_pl_ab_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "pl", .before = 1)) |> 
      bind_rows(lrt_contact_ab_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "contact", .before = 1)) |> 
      mutate(method = method_name, proxy = "ab", predictor = "oms", .before = 1)
    
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_ab_oms_flower_best <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_low_ab_oms <- summary(model_ab_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","pl","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_ab_oms_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_medium_ab_oms <- summary(model_ab_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("oms","pl","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_ab_oms_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_high_ab_oms <- summary(model_ab_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("oms","pl","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    ### seed-set ----
    model_ss_oms_flower_0 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_1 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ oms + ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_2 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ oms * ttt + pl + ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_3 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ oms * ttt + pl * ttt + contact + ttt + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_4 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ oms + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_4b <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_seed, nb_ov - nb_seed) ~ oms + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_4c <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_seed, nb_ov - nb_seed) ~ oms + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_5 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_6 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ oms + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_7 <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_seed, nb_ov - nb_seed) ~ oms + ttt + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_oms_flower_8 <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_seed, nb_ov - nb_seed) ~ oms + ttt + pl + (1|session) + (1|session:id), family = "binomial")
    lrt_interoms_ss_oms <- anova(model_ss_oms_flower_0, model_ss_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_ss_oms <- anova(model_ss_oms_flower_0, model_ss_oms_flower_2) |> as.data.frame() # pl * ttt
    lrt_intercontact_ss_oms <- anova(model_ss_oms_flower_0, model_ss_oms_flower_3) |> as.data.frame() # oms * ttt
    lrt_oms_ss_oms <- anova(model_ss_oms_flower_4, model_ss_oms_flower_5) |> as.data.frame() # oms
    lrt_ttt_ss_oms <-anova(model_ss_oms_flower_4, model_ss_oms_flower_6) |> as.data.frame() # ttt
    lrt_pl_ss_oms <-anova(model_ss_oms_flower_4b, model_ss_oms_flower_7) |> as.data.frame() # pl
    lrt_contact_ss_oms <-anova(model_ss_oms_flower_4c, model_ss_oms_flower_8) |> as.data.frame() # contact
    
    lrt_ss_oms <- lrt_interoms_ss_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interoms", .before = 1) |>
      bind_rows(lrt_interpl_ss_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interpl", .before = 1)) |>
      ...
      bind_rows(lrt_pl_ss_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "pl", .before = 1)) |> 
      bind_rows(lrt_contact_ss_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "contact", .before = 1)) |> 
      mutate(method = method_name, proxy = "ss", predictor = "oms", .before = 1)
    
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_ss_oms_flower_best <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_low_ss_oms <- summary(model_ss_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","pl","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_ss_oms_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_medium_ss_oms <- summary(model_ss_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("oms","pl","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_ss_oms_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_high_ss_oms <- summary(model_ss_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("oms","pl","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    model_germ_oms_flower_2 <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ oms * ttt + pl + ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_germ_oms_flower_3 <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ oms * ttt + pl * ttt + contact + ttt + (1|session) + (1|session:id), family = "binomial")
    model_germ_oms_flower_4 <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ oms + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_oms_flower_4b <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_germ, nb_sown - nb_germ) ~ oms + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_oms_flower_4c <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_germ, nb_sown - nb_germ) ~ oms + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_oms_flower_5 <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_oms_flower_6 <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ oms + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_oms_flower_7 <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_germ, nb_sown - nb_germ) ~ oms + ttt + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_oms_flower_8 <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_germ, nb_sown - nb_germ) ~ oms + ttt + pl + (1|session) + (1|session:id), family = "binomial")
    lrt_interoms_germ_oms <- anova(model_germ_oms_flower_0, model_germ_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_germ_oms <- anova(model_germ_oms_flower_0, model_germ_oms_flower_2) |> as.data.frame() # pl * ttt
    lrt_intercontact_germ_oms <- anova(model_germ_oms_flower_0, model_germ_oms_flower_3) |> as.data.frame() # oms * ttt
    lrt_oms_germ_oms <- anova(model_germ_oms_flower_4, model_germ_oms_flower_5) |> as.data.frame() # oms
    lrt_ttt_germ_oms <-anova(model_germ_oms_flower_4, model_germ_oms_flower_6) |> as.data.frame() # ttt
    lrt_pl_germ_oms <-anova(model_germ_oms_flower_4b, model_germ_oms_flower_7) |> as.data.frame() # pl
    lrt_contact_germ_oms <-anova(model_germ_oms_flower_4c, model_germ_oms_flower_8) |> as.data.frame() # contact
    
    lrt_germ_oms <- lrt_interoms_germ_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interoms", .before = 1) |>
      bind_rows(lrt_interpl_germ_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interpl", .before = 1)) |>
      ...
      bind_rows(lrt_pl_germ_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "pl", .before = 1)) |> 
      bind_rows(lrt_contact_germ_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "contact", .before = 1)) |> 
      mutate(method = method_name, proxy = "germ", predictor = "oms", .before = 1)
    
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_germ_oms_flower_best <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_low_germ_oms <- summary(model_germ_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","pl","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_germ_oms_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_medium_germ_oms <- summary(model_germ_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("oms","pl","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_germ_oms_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_high_germ_oms <- summary(model_germ_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("oms","pl","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    ### weight ----
    model_weight_oms_flower_0 <- lme4::lmer(data = data_final_flower, seed_weight ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    model_weight_oms_flower_1 <- lme4::lmer(data = data_final_flower, seed_weight ~ oms + ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    model_weight_oms_flower_2 <- lme4::lmer(data = data_final_flower, seed_weight ~ oms * ttt + pl + ttt + contact * ttt + (1|session) + (1|session:id))
    model_weight_oms_flower_3 <- lme4::lmer(data = data_final_flower, seed_weight ~ oms * ttt + pl * ttt + contact + ttt + (1|session) + (1|session:id))
    model_weight_oms_flower_4 <- lme4::lmer(data = data_final_flower, seed_weight ~ oms + ttt + pl + contact + (1|session) + (1|session:id))
    model_weight_oms_flower_4b <- lme4::lmer(data = data_final_flower |> filter(!is.na(pl)), seed_weight ~ oms + ttt + pl + contact + (1|session) + (1|session:id))
    model_weight_oms_flower_4c <- lme4::lmer(data = data_final_flower |> filter(!is.na(contact)), seed_weight ~ oms + ttt + pl + contact + (1|session) + (1|session:id))
    model_weight_oms_flower_5 <- lme4::lmer(data = data_final_flower, seed_weight ~ ttt + pl + contact + (1|session) + (1|session:id))
    model_weight_oms_flower_6 <- lme4::lmer(data = data_final_flower, seed_weight ~ oms + pl + contact + (1|session) + (1|session:id))
    model_weight_oms_flower_7 <- lme4::lmer(data = data_final_flower |> filter(!is.na(pl)), seed_weight ~ oms + ttt + contact + (1|session) + (1|session:id))
    model_weight_oms_flower_8 <- lme4::lmer(data = data_final_flower |> filter(!is.na(contact)), seed_weight ~ oms + ttt + pl + (1|session) + (1|session:id))
    lrt_interoms_weight_oms <- anova(model_weight_oms_flower_0, model_weight_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_weight_oms <- anova(model_weight_oms_flower_0, model_weight_oms_flower_2) |> as.data.frame() # pl * ttt
    lrt_intercontact_weight_oms <- anova(model_weight_oms_flower_0, model_weight_oms_flower_3) |> as.data.frame() # oms * ttt
    lrt_oms_weight_oms <- anova(model_weight_oms_flower_4, model_weight_oms_flower_5) |> as.data.frame() # oms
    lrt_ttt_weight_oms <-anova(model_weight_oms_flower_4, model_weight_oms_flower_6) |> as.data.frame() # ttt
    lrt_pl_weight_oms <-anova(model_weight_oms_flower_4b, model_weight_oms_flower_7) |> as.data.frame() # pl
    lrt_contact_weight_oms <-anova(model_weight_oms_flower_4c, model_weight_oms_flower_8) |> as.data.frame() # contact
    
    lrt_weight_oms <- lrt_interoms_weight_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interoms", .before = 1) |>
      bind_rows(lrt_interpl_weight_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interpl", .before = 1)) |>
      ...
      bind_rows(lrt_pl_weight_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "pl", .before = 1)) |> 
      bind_rows(lrt_contact_weight_oms |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "contact", .before = 1)) |> 
      mutate(method = method_name, proxy = "weight", predictor = "oms", .before = 1)
    
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_weight_oms_flower_best <- lmerTest::lmer(data = data_final_flower, seed_weight ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    estimate_low_weight_oms <- summary(model_weight_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","pl","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_weight_oms_flower_best <-  lmerTest::lmer(data = data_final_flower, seed_weight ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    estimate_medium_weight_oms <- summary(model_weight_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("oms","pl","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_weight_oms_flower_best <-  lmerTest::lmer(data = data_final_flower, seed_weight ~ oms * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    estimate_high_weight_oms <- summary(model_weight_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("oms","pl","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    estimate_weight_oms <- estimate_low_weight_oms |>
      ...
      mutate(method = method_name,
             predictor = "oms",
             proxy = "weight", .before = 1)
    
    ## effect of ratio_q on rs proxies ----
    ### aborted ----
    
    model_ab_q_flower_0 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_1 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q + ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_2 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q * ttt + pl + ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_3 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q * ttt + pl * ttt + contact + ttt + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_4 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_4b <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_seed,nb_ab) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_4c <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_seed,nb_ab) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_4d <- lme4::glmer(data = data_final_flower |> filter(!is.na(ratio_q)), cbind(nb_seed,nb_ab) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_5 <- lme4::glmer(data = data_final_flower |> filter (!is.na(ratio_q)), cbind(nb_seed,nb_ab) ~ ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_6 <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_7 <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_seed,nb_ab) ~ ratio_q + ttt + contact + (1|session) + (1|session:id), family = "binomial")
    model_ab_q_flower_8 <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_seed,nb_ab) ~ ratio_q + ttt + pl + (1|session) + (1|session:id), family = "binomial")
    lrt_interq_ab_q <- anova(model_ab_q_flower_0, model_ab_q_flower_1) |> as.data.frame() # ratio_q * ttt
    lrt_interpl_ab_q <- anova(model_ab_q_flower_0, model_ab_q_flower_2) |> as.data.frame() # pl * ttt
    lrt_intercontact_ab_q <- anova(model_ab_q_flower_0, model_ab_q_flower_3) |> as.data.frame() # ratio_q * ttt
    lrt_q_ab_q <- anova(model_ab_q_flower_4d, model_ab_q_flower_5) |> as.data.frame() # ratio_q
    lrt_ttt_ab_q <-anova(model_ab_q_flower_4, model_ab_q_flower_6) |> as.data.frame() # ttt
    lrt_pl_ab_q <-anova(model_ab_q_flower_4b, model_ab_q_flower_7) |> as.data.frame() # pl
    lrt_contact_ab_q <-anova(model_ab_q_flower_4c, model_ab_q_flower_8) |> as.data.frame() # contact
    
    lrt_ab_q <- lrt_interq_ab_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interq", .before = 1) |>
      bind_rows(lrt_interpl_ab_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interpl", .before = 1)) |>
      ...
      bind_rows(lrt_pl_ab_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "pl", .before = 1)) |> 
      bind_rows(lrt_contact_ab_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "contact", .before = 1)) |> 
      mutate(method = method_name, proxy = "ab", predictor = "ratio_q", .before = 1)
    
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_ab_q_flower_best <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_low_ab_q <- summary(model_ab_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","pl","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_ab_q_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_medium_ab_q <- summary(model_ab_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("ratio_q","pl","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_ab_q_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_high_ab_q <- summary(model_ab_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
    ...
    ### seed-set ----
    model_ss_q_flower_0 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_1 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q + ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_2 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q * ttt + pl + ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_3 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q * ttt + pl * ttt + contact + ttt + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_4 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_4b <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_4c <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_4d <- lme4::glmer(data = data_final_flower |> filter(!is.na(ratio_q)), cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_5 <- lme4::glmer(data = data_final_flower  |> filter(!is.na(ratio_q)), cbind(nb_seed, nb_ov - nb_seed) ~ ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_6 <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_7 <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q + ttt + contact + (1|session) + (1|session:id), family = "binomial")
    model_ss_q_flower_8 <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q + ttt + pl + (1|session) + (1|session:id), family = "binomial")
    lrt_interq_ss_q <- anova(model_ss_q_flower_0, model_ss_q_flower_1) |> as.data.frame() # ratio_q * ttt
    lrt_interpl_ss_q <- anova(model_ss_q_flower_0, model_ss_q_flower_2) |> as.data.frame() # pl * ttt
    lrt_intercontact_ss_q <- anova(model_ss_q_flower_0, model_ss_q_flower_3) |> as.data.frame() # ratio_q * ttt
    lrt_q_ss_q <- anova(model_ss_q_flower_4d, model_ss_q_flower_5) |> as.data.frame() # ratio_q
    lrt_ttt_ss_q <-anova(model_ss_q_flower_4, model_ss_q_flower_6) |> as.data.frame() # ttt
    lrt_pl_ss_q <-anova(model_ss_q_flower_4b, model_ss_q_flower_7) |> as.data.frame() # pl
    lrt_contact_ss_q <-anova(model_ss_q_flower_4c, model_ss_q_flower_8) |> as.data.frame() # contact
    
    lrt_ss_q <- lrt_interq_ss_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interq", .before = 1) |>
      bind_rows(lrt_interpl_ss_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interpl", .before = 1)) |>
      ...
      bind_rows(lrt_pl_ss_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "pl", .before = 1)) |> 
      bind_rows(lrt_contact_ss_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "contact", .before = 1)) |> 
      mutate(method = method_name, proxy = "ss", predictor = "ratio_q", .before = 1)
    
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_ss_q_flower_best <- lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_low_ss_q <- summary(model_ss_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","pl","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_ss_q_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_medium_ss_q <- summary(model_ss_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("ratio_q","pl","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_ss_q_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_seed, nb_ov - nb_seed) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_high_ss_q <- summary(model_ss_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("ratio_q","pl","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    model_germ_q_flower_2 <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q * ttt + pl + ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    model_germ_q_flower_3 <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q * ttt + pl * ttt + contact + ttt + (1|session) + (1|session:id), family = "binomial")
    model_germ_q_flower_4 <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_q_flower_4b <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_q_flower_4c <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_q_flower_4d <- lme4::glmer(data = data_final_flower |> filter(!is.na(ratio_q)), cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_q_flower_5 <- lme4::glmer(data = data_final_flower |> filter(!is.na(ratio_q)), cbind(nb_germ, nb_sown - nb_germ) ~ ttt + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_q_flower_6 <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q + pl + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_q_flower_7 <- lme4::glmer(data = data_final_flower |> filter(!is.na(pl)), cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q + ttt + contact + (1|session) + (1|session:id), family = "binomial")
    model_germ_q_flower_8 <- lme4::glmer(data = data_final_flower |> filter(!is.na(contact)), cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q + ttt + pl + (1|session) + (1|session:id), family = "binomial")
    lrt_interq_germ_q <- anova(model_germ_q_flower_0, model_germ_q_flower_1) |> as.data.frame() # ratio_q * ttt
    lrt_interpl_germ_q <- anova(model_germ_q_flower_0, model_germ_q_flower_2) |> as.data.frame() # pl * ttt
    lrt_intercontact_germ_q <- anova(model_germ_q_flower_0, model_germ_q_flower_3) |> as.data.frame() # ratio_q * ttt
    lrt_q_germ_q <- anova(model_germ_q_flower_4d, model_germ_q_flower_5) |> as.data.frame() # ratio_q
    lrt_ttt_germ_q <-anova(model_germ_q_flower_4, model_germ_q_flower_6) |> as.data.frame() # ttt
    lrt_pl_germ_q <-anova(model_germ_q_flower_4b, model_germ_q_flower_7) |> as.data.frame() # pl
    lrt_contact_germ_q <-anova(model_germ_q_flower_4c, model_germ_q_flower_8) |> as.data.frame() # contact
    
    lrt_germ_q <- lrt_interq_germ_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interq", .before = 1) |>
      bind_rows(lrt_interpl_germ_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interpl", .before = 1)) |>
      ...
      bind_rows(lrt_pl_germ_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "pl", .before = 1)) |> 
      bind_rows(lrt_contact_germ_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "contact", .before = 1)) |> 
      mutate(method = method_name, proxy = "germ", predictor = "ratio_q", .before = 1)
    
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_germ_q_flower_best <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_low_germ_q <- summary(model_germ_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","pl","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_germ_q_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_medium_germ_q <- summary(model_germ_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("ratio_q","pl","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_germ_q_flower_best <-  lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_high_germ_q <- summary(model_germ_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("ratio_q","pl","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    ### weight ----
    model_weight_q_flower_0 <- lme4::lmer(data = data_final_flower, seed_weight ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    model_weight_q_flower_1 <- lme4::lmer(data = data_final_flower, seed_weight ~ ratio_q + ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    model_weight_q_flower_2 <- lme4::lmer(data = data_final_flower, seed_weight ~ ratio_q * ttt + pl + ttt + contact * ttt + (1|session) + (1|session:id))
    model_weight_q_flower_3 <- lme4::lmer(data = data_final_flower, seed_weight ~ ratio_q * ttt + pl * ttt + contact + ttt + (1|session) + (1|session:id))
    model_weight_q_flower_4 <- lme4::lmer(data = data_final_flower, seed_weight ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id))
    model_weight_q_flower_4b <- lme4::lmer(data = data_final_flower |> filter(!is.na(pl)), seed_weight ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id))
    model_weight_q_flower_4c <- lme4::lmer(data = data_final_flower |> filter(!is.na(contact)), seed_weight ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id))
    model_weight_q_flower_4d <- lme4::lmer(data = data_final_flower |> filter(!is.na(ratio_q)), seed_weight ~ ratio_q + ttt + pl + contact + (1|session) + (1|session:id))
    model_weight_q_flower_5 <- lme4::lmer(data = data_final_flower |> filter(!is.na(ratio_q)), seed_weight ~ ttt + pl + contact + (1|session) + (1|session:id))
    model_weight_q_flower_6 <- lme4::lmer(data = data_final_flower, seed_weight ~ ratio_q + pl + contact + (1|session) + (1|session:id))
    model_weight_q_flower_7 <- lme4::lmer(data = data_final_flower |> filter(!is.na(pl)), seed_weight ~ ratio_q + ttt + contact + (1|session) + (1|session:id))
    model_weight_q_flower_8 <- lme4::lmer(data = data_final_flower |> filter(!is.na(contact)), seed_weight ~ ratio_q + ttt + pl + (1|session) + (1|session:id))
    lrt_interq_weight_q <- anova(model_weight_q_flower_0, model_weight_q_flower_1) |> as.data.frame() # ratio_q * ttt
    lrt_interpl_weight_q <- anova(model_weight_q_flower_0, model_weight_q_flower_2) |> as.data.frame() # pl * ttt
    lrt_intercontact_weight_q <- anova(model_weight_q_flower_0, model_weight_q_flower_3) |> as.data.frame() # ratio_q * ttt
    lrt_q_weight_q <- anova(model_weight_q_flower_4d, model_weight_q_flower_5) |> as.data.frame() # ratio_q
    lrt_ttt_weight_q <-anova(model_weight_q_flower_4, model_weight_q_flower_6) |> as.data.frame() # ttt
    lrt_pl_weight_q <-anova(model_weight_q_flower_4b, model_weight_q_flower_7) |> as.data.frame() # pl
    lrt_contact_weight_q <-anova(model_weight_q_flower_4c, model_weight_q_flower_8) |> as.data.frame() # contact
    
    lrt_weight_q <- lrt_interq_weight_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interq", .before = 1) |>
      bind_rows(lrt_interpl_weight_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "interpl", .before = 1)) |>
      ...
      bind_rows(lrt_pl_weight_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "pl", .before = 1)) |> 
      bind_rows(lrt_contact_weight_q |> slice(2) |> select(c("Chisq","Df","Pr(>Chisq)")) |> mutate(effect = "contact", .before = 1)) |> 
      mutate(method = method_name, proxy = "weight", predictor = "ratio_q", .before = 1)
    
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_weight_q_flower_best <- lmerTest::lmer(data = data_final_flower, seed_weight ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    estimate_low_weight_q <- summary(model_weight_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","pl","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_weight_q_flower_best <-  lmerTest::lmer(data = data_final_flower, seed_weight ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    estimate_medium_weight_q <- summary(model_weight_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("ratio_q","pl","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_weight_q_flower_best <-  lmerTest::lmer(data = data_final_flower, seed_weight ~ ratio_q * ttt + pl * ttt + contact * ttt + (1|session) + (1|session:id))
    estimate_high_weight_q <- summary(model_weight_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in%  c("ratio_q","pl","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    model_q_oms_flower_2 <- lme4::lmer(data = data_final_flower, ratio_q ~ oms * ttt + pl + ttt + (1|session) + (1|session:id))
    ...
    lrt_interoms_q_oms <- anova(model_q_oms_flower_0, model_q_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_q_oms <- anova(model_q_oms_flower_0, model_q_oms_flower_2) |> as.data.frame() # pl * ttt
    lrt_oms_q_oms <- anova(model_q_oms_flower_3, model_q_oms_flower_4) |> as.data.frame() # oms
    lrt_ttt_q_oms <-anova(model_q_oms_flower_3, model_q_oms_flower_5) |> as.data.frame() # ttt
    lrt_pl_q_oms <-anova(model_q_oms_flower_3b, model_q_oms_flower_6) |> as.data.frame() # pl
    ...
    lrt_interoms_ab_oms <- anova(model_ab_oms_flower_0, model_ab_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_ab_oms <- anova(model_ab_oms_flower_0, model_ab_oms_flower_1) |> as.data.frame() # pl * ttt
    lrt_oms_ab_oms <- anova(model_ab_oms_flower_3, model_ab_oms_flower_4) |> as.data.frame() # oms
    lrt_ttt_ab_oms <-anova(model_ab_oms_flower_3, model_ab_oms_flower_5) |> as.data.frame() # ttt
    lrt_pl_ab_oms <-anova(model_ab_oms_flower_3b, model_ab_oms_flower_6) |> as.data.frame() # pl
    ...
    lrt_interoms_ss_oms <- anova(model_ss_oms_flower_0, model_ss_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_ss_oms <- anova(model_ss_oms_flower_0, model_ss_oms_flower_1) |> as.data.frame() # pl * ttt
    lrt_oms_ss_oms <- anova(model_ss_oms_flower_3, model_ss_oms_flower_4) |> as.data.frame() # oms
    lrt_ttt_ss_oms <-anova(model_ss_oms_flower_3, model_ss_oms_flower_5) |> as.data.frame() # ttt
    lrt_pl_ss_oms <-anova(model_ss_oms_flower_3b, model_ss_oms_flower_6) |> as.data.frame() # pl
    ...
    lrt_interoms_germ_oms <- anova(model_germ_oms_flower_0, model_germ_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_germ_oms <- anova(model_germ_oms_flower_0, model_germ_oms_flower_1) |> as.data.frame() # pl * ttt
    lrt_oms_germ_oms <- anova(model_germ_oms_flower_3, model_germ_oms_flower_4) |> as.data.frame() # oms
    lrt_ttt_germ_oms <-anova(model_germ_oms_flower_3, model_germ_oms_flower_5) |> as.data.frame() # ttt
    lrt_pl_germ_oms <-anova(model_germ_oms_flower_3b, model_germ_oms_flower_6) |> as.data.frame() # pl
    ...
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_germ_oms_flower_best <- lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ oms * ttt + pl * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_low_germ_oms <- summary(model_germ_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","pl")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    ...
    lrt_interoms_weight_oms <- anova(model_weight_oms_flower_0, model_weight_oms_flower_1) |> as.data.frame() # oms * ttt
    lrt_interpl_weight_oms <- anova(model_weight_oms_flower_0, model_weight_oms_flower_1) |> as.data.frame() # pl * ttt
    lrt_oms_weight_oms <- anova(model_weight_oms_flower_3, model_weight_oms_flower_4) |> as.data.frame() # oms
    lrt_ttt_weight_oms <-anova(model_weight_oms_flower_3, model_weight_oms_flower_5) |> as.data.frame() # ttt
    lrt_pl_weight_oms <-anova(model_weight_oms_flower_3b, model_weight_oms_flower_6) |> as.data.frame() # pl
    ...
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_weight_oms_flower_best <-lmerTest::lmer(data = data_final_flower, seed_weight ~ oms * ttt + pl * ttt + (1|session) + (1|session:id))
    estimate_low_weight_oms <- summary(model_weight_oms_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","pl")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    ...
    lrt_interq_ab_q <- anova(model_ab_q_flower_0, model_ab_q_flower_1) |> as.data.frame() # ratio_q * ttt
    lrt_interpl_ab_q <- anova(model_ab_q_flower_0, model_ab_q_flower_1) |> as.data.frame() # pl * ttt
    lrt_q_ab_q <- anova(model_ab_q_flower_3a, model_ab_q_flower_4) |> as.data.frame() # ratio_q
    lrt_ttt_ab_q <-anova(model_ab_q_flower_3, model_ab_q_flower_5) |> as.data.frame() # ttt
    lrt_pl_ab_q <-anova(model_ab_q_flower_3b, model_ab_q_flower_6) |> as.data.frame() # pl
    ...
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_ab_q_flower_best <- lme4::glmer(data = data_final_flower, cbind(nb_seed,nb_ab) ~ ratio_q * ttt + pl * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_low_ab_q <- summary(model_ab_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
    ...
    lrt_interq_ss_q <- anova(model_ss_q_flower_0, model_ss_q_flower_1) |> as.data.frame() # ratio_q * ttt
    lrt_interpl_ss_q <- anova(model_ss_q_flower_0, model_ss_q_flower_1) |> as.data.frame() # pl * ttt
    lrt_q_ss_q <- anova(model_ss_q_flower_3a, model_ss_q_flower_4) |> as.data.frame() # ratio_q
    lrt_ttt_ss_q <-anova(model_ss_q_flower_3, model_ss_q_flower_5) |> as.data.frame() # ttt
    lrt_pl_ss_q <-anova(model_ss_q_flower_3b, model_ss_q_flower_6) |> as.data.frame() # pl
    ...
    lrt_interq_germ_q <- anova(model_germ_q_flower_0, model_germ_q_flower_1) |> as.data.frame() # ratio_q * ttt
    lrt_interpl_germ_q <- anova(model_germ_q_flower_0, model_germ_q_flower_1) |> as.data.frame() # pl * ttt
    lrt_q_germ_q <- anova(model_germ_q_flower_3a, model_germ_q_flower_4) |> as.data.frame() # ratio_q
    lrt_ttt_germ_q <-anova(model_germ_q_flower_3, model_germ_q_flower_5) |> as.data.frame() # ttt
    lrt_pl_germ_q <-anova(model_germ_q_flower_3b, model_germ_q_flower_6) |> as.data.frame() # pl
    ...
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "low"))
    model_germ_q_flower_best <-lme4::glmer(data = data_final_flower, cbind(nb_germ, nb_sown - nb_germ) ~ ratio_q * ttt + pl * ttt + (1|session) + (1|session:id), family = "binomial")
    estimate_low_germ_q <- summary(model_germ_q_flower_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","pl")) |> mutate(ttt = "low", .before = 1)
    data_final_flower <- data_final_flower |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    ...
    lrt_interq_weight_q <- anova(model_weight_q_flower_0, model_weight_q_flower_1) |> as.data.frame() # ratio_q * ttt
    lrt_interpl_weight_q <- anova(model_weight_q_flower_0, model_weight_q_flower_1) |> as.data.frame() # pl * ttt
    lrt_q_weight_q <- anova(model_weight_q_flower_3a, model_weight_q_flower_4) |> as.data.frame() # ratio_q
    lrt_ttt_weight_q <-anova(model_weight_q_flower_3, model_weight_q_flower_5) |> as.data.frame() # ttt
    lrt_pl_weight_q <-anova(model_weight_q_flower_3b, model_weight_q_flower_6) |> as.data.frame() # pl
...
get_stat_id <- function(data_final_id, include_contact = FALSE){
  ...
  if(include_contact){
    ...
    lrt_intercontact_q_oms <- anova(model_q_oms_id_0, model_q_oms_id_2) |> as.data.frame() # contact * ttt
    ...
    estimate_low_q_oms <- summary(model_q_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_q_oms_id_best <- lmerTest::lmer(data = data_final_id, ratio_q ~ oms * ttt + contact * ttt + (1|session))
    estimate_medium_q_oms <- summary(model_q_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_q_oms_id_best <- lmerTest::lmer(data = data_final_id, ratio_q ~ oms * ttt + contact * ttt + (1|session))
    estimate_high_q_oms <- summary(model_q_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
    ...
    lrt_intercontact_ab_oms <- anova(model_ab_oms_id_0, model_ab_oms_id_2) |> as.data.frame() # contact * ttt
    ...
    estimate_low_ab_oms <- summary(model_ab_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_ab_oms_id_best <-  lme4::glmer(data = data_final_id, cbind(nb_seed_sum_ab,nb_ab_sum_ab) ~ oms * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_medium_ab_oms <- summary(model_ab_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_ab_oms_id_best <- lme4::glmer(data = data_final_id, cbind(nb_seed_sum_ab,nb_ab_sum_ab) ~ oms * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_high_ab_oms <- summary(model_ab_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    lrt_intercontact_ss_oms <- anova(model_ss_oms_id_0, model_ss_oms_id_2) |> as.data.frame() # contact * ttt
    ...
    estimate_low_ss_oms <- summary(model_ss_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_ss_oms_id_best <- lme4::glmer(data = data_final_id, cbind(nb_seed_sum_ss, nb_ov_sum_ss - nb_seed_sum_ss) ~ oms * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_medium_ss_oms <- summary(model_ss_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_ss_oms_id_best <- lme4::glmer(data = data_final_id, cbind(nb_seed_sum_ss, nb_ov_sum_ss - nb_seed_sum_ss) ~ oms * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_high_ss_oms <- summary(model_ss_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    lrt_intercontact_germ_oms <- anova(model_germ_oms_id_0, model_germ_oms_id_2) |> as.data.frame() # contact * ttt
    ...
    estimate_low_germ_oms <- summary(model_germ_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_germ_oms_id_best <- lme4::glmer(data = data_final_id, cbind(nb_germ_sum_germ, nb_sown_sum_germ - nb_germ_sum_germ) ~ oms * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_medium_germ_oms <- summary(model_germ_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_germ_oms_id_best <- lme4::glmer(data = data_final_id, cbind(nb_germ_sum_germ, nb_sown_sum_germ - nb_germ_sum_germ) ~ oms * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_high_germ_oms <- summary(model_germ_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    lrt_intercontact_weight_oms <- anova(model_weight_oms_id_0, model_weight_oms_id_2) |> as.data.frame() # contact * ttt
    ...
    estimate_low_weight_oms <- summary(model_weight_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_weight_oms_id_best <- lmerTest::lmer(data = data_final_id, mean_seed_weight ~ oms * ttt + contact * ttt + (1|session))
    estimate_medium_weight_oms <- summary(model_weight_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_weight_oms_id_best <- lmerTest::lmer(data = data_final_id, mean_seed_weight ~ oms * ttt + contact * ttt + (1|session))
    estimate_high_weight_oms <- summary(model_weight_oms_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("oms","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    lrt_intercontact_ab_q <- anova(model_ab_q_id_0, model_ab_q_id_2) |> as.data.frame() # contact * ttt
    ...
    estimate_low_ab_q <- summary(model_ab_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_ab_q_id_best <- lme4::glmer(data = data_final_id, cbind(nb_seed_sum_ab,nb_ab_sum_ab) ~ ratio_q * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_medium_ab_q <- summary(model_ab_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_ab_q_id_best <- lme4::glmer(data = data_final_id, cbind(nb_seed_sum_ab,nb_ab_sum_ab) ~ ratio_q * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_high_ab_q <- summary(model_ab_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
    ...
    lrt_intercontact_ss_q <- anova(model_ss_q_id_0, model_ss_q_id_2) |> as.data.frame() # contact * ttt
    ...
    estimate_low_ss_q <- summary(model_ss_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_ss_q_id_best <- lme4::glmer(data = data_final_id, cbind(nb_seed_sum_ss, nb_ov_sum_ss - nb_seed_sum_ss) ~ ratio_q * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_medium_ss_q <- summary(model_ss_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_ss_q_id_best <- lme4::glmer(data = data_final_id, cbind(nb_seed_sum_ss, nb_ov_sum_ss - nb_seed_sum_ss) ~ ratio_q * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_high_ss_q <- summary(model_ss_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    lrt_intercontact_germ_q <- anova(model_germ_q_id_0, model_germ_q_id_2) |> as.data.frame() # contact * ttt
    ...
    estimate_low_germ_q <- summary(model_germ_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_germ_q_id_best <- lme4::glmer(data = data_final_id, cbind(nb_germ_sum_germ, nb_sown_sum_germ - nb_germ_sum_germ) ~ ratio_q * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_medium_germ_q <- summary(model_germ_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_germ_q_id_best <- lme4::glmer(data = data_final_id, cbind(nb_germ_sum_germ, nb_sown_sum_germ - nb_germ_sum_germ) ~ ratio_q * ttt + contact * ttt + (1|session), family = "binomial")
    estimate_high_germ_q <- summary(model_germ_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "high", .before = 1)
    ...
    lrt_intercontact_weight_q <- anova(model_weight_q_id_0, model_weight_q_id_2) |> as.data.frame() # contact * ttt
    ...
    estimate_low_weight_q <- summary(model_weight_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "low", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "medium"))
    model_weight_q_id_best <- lmerTest::lmer(data = data_final_id, mean_seed_weight ~ ratio_q * ttt + contact * ttt + (1|session))
    estimate_medium_weight_q <- summary(model_weight_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "medium", .before = 1)
    data_final_id <- data_final_id |>
      mutate(ttt=forcats::fct_relevel(ttt, "high"))
    model_weight_q_id_best <- lmerTest::lmer(data = data_final_id, mean_seed_weight ~ ratio_q * ttt + contact * ttt + (1|session))
    estimate_high_weight_q <- summary(model_weight_q_id_best)$coefficients |> as.data.frame() |> tibble::rownames_to_column() |>
      filter(rowname %in% c("ratio_q","contact")) |> mutate(ttt = "high", .before = 1)
